package com.example.mgp2021;

// Created by TanSiewLan2021

// Able to help you do layering of different assets rendered on the screen

public class LayerConstants {

    public final static int BACKGROUND_LAYER = 0;
    public final static int RENDERTEXT_LAYER = 32;
    public final static int STAR_LAYER = 2;
    public final static int BULLET_LAYER = 3;
    public final static int PLAYER_LAYER = 4;
    public final static int ENEMY_LAYER = 5;
    public final static int COLLECTIBLES_LAYER = 6;
    public final static int PAUSEBUTTON_LAYER = 30;
    public final static int HEALTHBAR_LAYER = 31;
    public final static int GAMEOBJECTS_LAYER = 100;  // Example
    public final static int UI_LAYER = 1000;
}